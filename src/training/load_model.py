...
def load_model(
    ...
    force_different_tokenizer: bool = False,
    different_tokenizer_name_or_path: Optional[str] = None,
    ...
    # Sanity checks
    if force_different_tokenizer and different_tokenizer_name_or_path is None:
        raise ValueError(
            "You set force_different_tokenizer to True but you did not provide a different_tokenizer_name_or_path"
        )
    ...
    if use_lora:
        print(f"Loading LoRA weights from {lora_weights_name_or_path}")
    ...
    if force_different_tokenizer and different_tokenizer_name_or_path is not None:
        logging.info(
            f"Loading a different tokenizer from {different_tokenizer_name_or_path}"
        )
        tokenizer: PreTrainedTokenizerBase = AutoTokenizer.from_pretrained(
            different_tokenizer_name_or_path,
            add_eos_token="solar" in different_tokenizer_name_or_path.lower(),
            trust_remote_code=trust_remote_code,
        )
    else:
        tokenizer: PreTrainedTokenizerBase = AutoTokenizer.from_pretrained(
    ...
    if "gemma-3" in model_weights_name_or_path:
        kwargs = {"attn_implementation": "eager"}
        if use_flash_attention:
            logging.warning(
                "Gemma3 models are not recomended with flash attention. We will load the model with eager attention."
            )
        logging.info("Loading the model with eager attention")
    if cache_dir is not None:
        kwargs["cache_dir"] = cache_dir
        logging.info(f"Loading the model with cache dir: {cache_dir}")
    #  Load the model weights
    ...
    logging.info("READY TO LOAD MODEL")
    ...
    if max_seq_length is not None:
        try:
            ...
            logging.warning(
                f"Invalid max_seq_length={max_seq_length}. Using model max_position_embeddings instead."
            )
            max_seq_length = None
    if max_seq_length is not None:
        model_max_len = min(model_max_len, max_seq_length)
        logging.info(f"Overriding tokenizer.model_max_length to {model_max_len}")
