...
def compute_metrics_crf(p, SPECIAL_TOKENS_ASSISTANT_START, SPECIAL_TOKENS,  items_list_in_dataset_validation:list=[], items_list_in_dataset_test:list=[], calc_per_item=True, verbose=True):
    predictions, labels = p
    labels = labels[0]
    # predictions are already argmax'd by preprocess_logits_for_metrics
    # shape: (num_samples, seq_len) instead of (num_samples, seq_len, vocab_size)
    if verbose:
    ...
    # print('len(true_predictions[0]):\n',len(true_predictions[0]))
    # print('true_labels[0]:\n',true_labels[0])
    # print('true_predictions[0]:\n',true_predictions[0])
    # get only the predicted part, not the input prompt, separated by the special tokens sequence
    true_predictions = [tp[-(len(tl)): ] for tp, tl in zip(true_predictions, true_labels)]
    # true_labels = [tl for tl in true_labels]
    ...
    # print('true_predictions[0]: ', true_predictions[0])
    # print('true_labels[0]: ', true_labels[0])   
    ...
    if seqeval:
        ...
        # if verbose:
        #     print('map_for_eval_fn: ', map_for_eval_fn)
    # print('true_predictions: ', true_predictions)

    # tmp_for_print_label = set(true_labels)
    # tmp_for_print_pred = set(true_predictions)
    # print(f'true_labels have {len(tmp_for_print_label)} unique values: ', tmp_for_print_label)
    # print(f'true_predictions have {len(tmp_for_print_pred)} unique values: ', tmp_for_print_pred)
    ...
    if True:
        ...
        if calc_per_item:
            ...
            for item in unique_items:
                ...
                # true_predictions/true_labels are already mapped to integer ids above.
    ...
    #     print(f"Error calculating F1 scores per item: {e}")
...
def compute_accuracy_medqa(p, SPECIAL_TOKENS_ASSISTANT_START, SPECIAL_TOKENS):
    predictions, labels = p
    labels = labels[0]
    # predictions are already argmax'd by preprocess_logits_for_metrics
    # shape: (num_samples, seq_len) instead of (num_samples, seq_len, vocab_size)
    ...
    true_predictions = [''.join(map(str, l)) for l in true_predictions]
    true_predictions = [int(tok) if tok else -1 for tok in true_predictions]
    true_labels = [''.join(map(str, l)) for l in true_labels]
    true_labels = [int(tok) if tok else -2 for tok in true_labels]
    print('One prediction: ', true_predictions[0])
    print('One label: ', true_labels[0])
    map_for_eval_fn = {real_val: i for i, real_val in enumerate(set(list(true_labels) + list(true_predictions)))}
    map_for_eval_fn['INVALID'] = -1
    true_labels = [map_for_eval_fn[real_val] for real_val in true_labels]
    true_predictions = [map_for_eval_fn[real_val] for real_val in true_predictions]
    # print('true_predictions: ', true_predictions)
    ...
    results_micro = accuracy.compute(predictions=true_predictions, references=true_labels)
    results = {
        "accuracy": results_micro["accuracy"]
    }
    #print(results)
    return results


def compute_accuracy_qa(p, SPECIAL_TOKENS_ASSISTANT_START, SPECIAL_TOKENS):
    predictions, labels = p
    labels = labels[0]
    # predictions are already argmax'd by preprocess_logits_for_metrics
    # shape: (num_samples, seq_len) instead of (num_samples, seq_len, vocab_size)
    ...
    ## CALCULATE WEAK ACCURACY, i.e. if the predicted answer contains the correct answer as a substring, it is considered correct
    TP_weak = sum(1 for pred, label in zip(true_predictions, true_labels) if str(label).lower() in str(pred).lower() or str(pred).lower() in str(label).lower())
    TP_exact = sum(1 for pred, label in zip(true_predictions, true_labels) if pred == label)
    results = {
        "accuracy_exact_match": TP_exact / len(true_labels),
        "accuracy_weak": TP_weak / len(true_labels)
    }
    #print(results)
    return results