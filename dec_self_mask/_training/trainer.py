...
class MedLlmTrainer(Seq2SeqTrainer):
    """
    The MedLlmTrainer is an adaptation of the 🤗 Transformers Trainer.
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        # Print a compact preview of the first batch only on global rank 0.
        # Large per-rank prints can desynchronize DDP workers and trigger NCCL timeouts.
        if (
            ...
            if self.tokenizer is not None:
                ...
                # )
                # print([decoded[0][:1000]])
                # token_ids_preview = print_input_ids[0][:64].tolist()
                # token_strings_preview = [
                #     self.processing_class.decode(int(tkn)) for tkn in token_ids_preview
                # ]
                # print("token_ids_preview(64):", token_ids_preview)
                # print("token_strings_preview(64):", token_strings_preview)
                print(self.tokenizer.batch_decode(print_input_ids))
                decoded_tkn_by_tnk = [self.processing_class.decode(tkn) for tkn in print_input_ids[0]]
                print(decoded_tkn_by_tnk)
            else:
